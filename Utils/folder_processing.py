...
def folder_processing(original_path_or_dir: str, out_path_or_dir: str) -> None:
    """
    ...
    for file in os.listdir(original_path_or_dir):
        if file.endswith(".min"):
            ...
            make_csv_from_original_data(original_path_or_dir + file, out_csv, comments_end=23)
            replace_spaces_with_semicolon(out_csv)
            counter += 1
    print (f"Processing is over.\nThere were processed {counter} files with {(time.time() - start_time).round(3)} seconds.")